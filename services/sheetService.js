  ...
  async getOrders() {
    try {
      ...
      return rows.slice(1).map(row => {
        const get = (n) => row[headers.indexOf(n)] || '';
        return { orderId: get('order_ID'), buyerId: get('buyer_ID'), buyerName: get('buyer_name'), productCode: get('product_code'), productName: get('product_name'), spec: get('spec'), qty: get('qty'), price: get('price'), total: get('total'), orderDate: get('order_date'), status: get('status') };
      });
    } catch (e) { return []; }
  },
